...
const TelaEditarRecorrencia = ({ navigation, route }) => {
  const { recorrenciaId } = route.params;
  const { atualizarRecorrencia, isSaving, validarRecorrencia } = useGerenciarRecorrencias();
   ...
   */
  useEffect(() => {
    const carregarRecorrencia = async () => {
      try {
        const dados = await recorrenciaService.buscarPorId(recorrenciaId);
        setRecorrencia(dados);
        setFormData({
          descricao: dados.descricao,
          frequencia: dados.frequencia,
          intervalo: dados.intervalo,
          dia_inicio: dados.dia_inicio,
          tipo_limite: dados.tipo_limite,
          data_fim: dados.data_fim,
          qtd_ocorrencias: dados.qtd_ocorrencias,
          valor: dados.valor.toString(),
          tipo: dados.tipo,
          fk_categoria: dados.fk_categoria,
          fk_instituicao: dados.fk_instituicao,
          ativo: dados.ativo,
        });
      } catch (erro) {
        Alert.alert('Erro ao carregar', 'Não foi possível carregar a recorrência');
        navigation.goBack();
      } finally {
        setLoading(false);
      }
  ...
  const handleSalvar = useCallback(async () => {
    ...
    const atualizada = await atualizarRecorrencia(recorrenciaId, payload);
    if (atualizada) {
      navigation.goBack();
    }
  }, [formData, validarRecorrencia, atualizarRecorrencia, validarForm, recorrenciaId, navigation]);
...
export default TelaEditarRecorrencia;
