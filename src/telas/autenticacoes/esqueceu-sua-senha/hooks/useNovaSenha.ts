...
export interface UseNovaSenhaResult {
  ...
  loading: boolean;
  error: string | null;
  handleAtualizar: (navigation: { navigate: (route: string) => void }) => Promise<boolean>;
}
...
export function useNovaSenha(token: string): UseNovaSenhaResult {
  ...
  const handleAtualizar = async (navigation: { navigate: (route: string) => void }): Promise<boolean> => {
    ...
    try {
      ...
      setError(
        status && status >= 400 && status < 500
          ...
          : (msg ? String(msg) : 'Falha ao atualizar senha. Tente novamente.')
      );
      setLoading(false);
      return false;
    }
