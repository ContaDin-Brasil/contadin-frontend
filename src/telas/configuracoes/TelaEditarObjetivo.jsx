...
const TelaEditarObjetivo = ({ navigation, route }) => {
  const objetivoInicial = route.params?.objetivo || OBJETIVO_MOCK;
  ...
  const [categoria, setCategoria] = useState(objetivoInicial.categoria);
  const [valorAlvo, setValorAlvo] = useState(String(objetivoInicial.valorAlvo));
  ...
  const handleSalvar = () => {
    ...
    abrirAviso('Objetivo atualizado', 'As alterações foram salvas.', () => navigation.goBack());
  };
